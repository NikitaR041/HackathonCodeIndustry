...
ifndef SCENARIO
SCENARIO = $(wildcard *.feature)
endif

CPPLIST = $(wildcard *.cpp)
CSOURCES = $(wildcard *.c)
CPPSOURCES = $(filter-out %_unittest.cpp %_featuretest.cpp,$(CPPLIST))

TESTLIST = $(patsubst %.cpp,%,$(filter %_unittest.cpp,$(CPPLIST)))
TESTLISTEXE = $(patsubst %,$(TARGET)/%.exe,$(TESTLIST))
FEATURELISTEXE = $(patsubst %.cpp,$(TARGET)/%.exe,$(filter %_featuretest.cpp,$(CPPLIST)))
...
TESTLIBS = gmock gherkin-cpp units gcov
...
CPPLIBS =  conio2 nuklear bgi gdi32 msimg32 comdlg32 uuid oleaut32 ole32 reflex $(TESTLIBS)
CLIBS =  conio2 nuklear gdi32 msimg32
ifneq (,$(strip $(CPPSOURCES) $(LEXSOURCE) $(GRMSOURCE) $(CAIOSOURCES) $(SMSOURCES)))
CC = $(GPP)
LIBS = $(CPPLIBS) $(TESTLIBS)
...
INIT="C:\Users\volog\OneDrive\Desktop\MinIDE\SciTE/../lib/initansicp.o"
...
.PHONY : all clean cleangcda featuretests unittests $(TESTLIST)
...
$(TARGET)/_all_.exe $(OBJS) $(GRMGENER) $(GRMHEADER) $(LEXGENER) $(CAIOGENER) $(SMGENER) $(TESTLIST) features: | $(TARGET)
...
clean:
	...
	$(if $(wildcard gcm.cache),rmdir /q /s gcm.cache)
...
$(TARGET)/libunits.a: $(OBJS) Makefile
	$(if $(wildcard $(TARGET)/libunits.a),del $(TARGET)\\libunits.a)
	ar -rv $(TARGET)/libunits.a $(OBJS)

featuretests: $(FEATURELISTEXE)
	set SCENARIO=$(SCENARIO) && runall.cmd $(subst /,\\,$^)

unittests: $(TESTLISTEXE)
	runall.cmd $(subst /,\\,$^)


$(TARGET)/%_unittest.exe: %_unittest.cpp $(TARGET)/libunits.a $(CPPHEADERS) $(CHEADERS)
	$(CC) $(CCFLAGS) $(LFLAGS) -L$(TARGET) -o $@  $< $(INIT) $(addprefix -l,$(LIBS))

$(TARGET)/%_featuretest.exe: %_featuretest.cpp $(TARGET)/libunits.a $(CPPHEADERS) $(CHEADERS)
	$(CC) $(CCFLAGS) $(LFLAGS) -L$(TARGET) -o $@  $< $(INIT) $(addprefix -l,$(LIBS))
